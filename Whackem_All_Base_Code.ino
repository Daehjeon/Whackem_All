...
{  
  ...
  for(i=0;i < TS_Length;i++){  
    ...
    //Before detecting optic encoder
    While (Received == 0){
      Step_Input = 300;
      OLStep();
      
      (!Stop && !Stop_bit) ? Move_Motor(Controller_Output) : Stop_Motor();
    }

    //After detecting optic encoder
    if (Received == 1){
      Reference_Input_Encoder = Received_Position;
      while (!Done){
